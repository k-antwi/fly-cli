...
class InstallVueCommand extends Command
{
    ...
    protected $description = "Install Fly's Docker Compose file for Vue.js projects";

    protected $vueDefaultServices = ['mailpit'];

    protected $vueServices = [
        'mysql',
        'pgsql',
        'mariadb',
        'mongodb',
    ...
    public function handle(): int
    {
        ...
        if ($invalidServices = array_diff($services, $this->vueServices)) {
            $this->components->error('Invalid services ['.implode(',', $invalidServices).'].');

            return 1;
        }

        $this->publishVueDockerResources();
        $this->buildVueDockerCompose($services);
    ...
    private function installRemotely(): int
    {
        ...
        $this->publishVueDockerResources();
        $this->buildVueDockerComposeForProduction($services);
    ...
    private function resolveServices(): array
    {
        ...
        if ($this->option('no-interaction')) {
            return $this->vueDefaultServices;
        }

        return $this->gatherVueServicesInteractively();
    }

    private function gatherVueServicesInteractively(): array
    {
        if (function_exists('\Laravel\Prompts\multiselect')) {
            return \Laravel\Prompts\multiselect(
                label: 'Which services would you like to install?',
                options: $this->vueServices,
                default: [],
            );
        }

        return $this->choice('Which services would you like to install?', $this->vueServices, 0, null, true);
    }
}
